...
def scout_custom_targets(ctx, player_name, max_age=24, max_budget_eur=None, top_n=5, same_pos_only=False):
    df = ctx['df']
    df_ml = ctx['df_ml']
    X_pca = ctx['X_pca']
    ...
    if len(player_matches) == 0:
        return f"Cannot find a player with this name: {player_name}", None
...
# ==========================================
# درع حماية الـ Sliders من إعدادات لغة المتصفح
# ==========================================
st.markdown("""
    <style>
    /* إجبار السلايدر وجميع عناصره الداخلية على العمل باتجاه اليسار لليمين */
...
with tab_classifier:
    ...
    st.markdown("Enter the detailed stats of the player, and the AI will predict the best position.")

    # ==========================================
    # 2. Load Models and Transformers
    # ==========================================
    @st.cache_resource
    def load_models():
        try:
            ...
            st.error("⚠️ Please make sure files (scaler.pkl, pca_final.pkl, xgb_model.pkl, label_encoder.pkl) are in the path.")
            st.stop()
    ...
    # 3. User Interface (UI)
    # ==========================================
    # Split the screen into Tabs for easier input
    ...
    if st.button("🚀 Analyze & Predict Position", use_container_width=True):
        with st.spinner("Analyzing player attributes..."):
            ...
            # 1. StandardScaler for all columns (44 columns)
            scaled_array = scaler.transform(df_input)

            # 2. Separate main columns (first 15) from sub columns (last 29)
            main_scaled = scaled_array[:, :15]
            sub_scaled = scaled_array[:, 15:]

            # 3. Apply PCA on sub columns only
            sub_pca = pca_final.transform(sub_scaled)

            # 4. Combine main columns with PCA output
            ...
            # Convert output to explicit Integer to avoid sklearn conflicts
            pred_idx = int(prediction_encoded[0])

            # 6. Decode the predicted index to position name
